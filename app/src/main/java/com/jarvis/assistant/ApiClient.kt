...
object ApiClient {
        ...
        withContext(Dispatchers.IO) {
            ...
            when (commandResult) {
                is JarvisCommandParser.CommandResult.Executed -> {
                    val text = if (commandResult.outputMessage.contains(JarvisCommandParser.CONTACT_FORMAT_MARKER)) {
                        val formatted = applyContactFormatting(context, provider, commandResult.outputMessage)
                ...
                is JarvisCommandParser.CommandResult.ExecutedMultiple -> {
                    ...
                    val text = if (combined.contains(JarvisCommandParser.CONTACT_FORMAT_MARKER)) {
                        val formatted = applyContactFormatting(context, provider, combined)
                        if (cleanText.isBlank()) formatted else "$cleanText\n\n$formatted"
    ...
    /**
     * Reformate une fiche/liste de contacts selon la consigne détectée via le marqueur
     * JarvisCommandParser.CONTACT_FORMAT_MARKER (consigne persistée via
     * set_contact_presentation_style et/ou demande ponctuelle format_hint) — c'est le SEUL
     * point d'entrée qui peut réellement changer la présentation par défaut de
     * PeopleController, puisque search_contact_profile/list_contacts_by_category ne passent
     * pas par summarizeNaturally (qui produit une réponse orale sans mise en forme, incompatible
     * avec une fiche visuelle en sections/emojis). Contrairement à summarizeNaturally, ce prompt
     * autorise explicitement les retours à la ligne, sections et emojis.
     */
    private suspend fun applyContactFormatting(context: Context, provider: Provider, rawWithInstruction: String): String {
        val marker = JarvisCommandParser.CONTACT_FORMAT_MARKER
        ...
        val prompt =
            "Voici une fiche contact (ou une liste de fiches), avec des données EXACTES et RÉELLES " +
                "extraites de l'application :\n\n$data\n\n" +
                "Consigne(s) de présentation à appliquer : $instruction\n\n" +
                "Réécris cette fiche en respectant ces consignes. Règle absolue : ne change, n'ajoute, " +
                "ne devine et n'omets AUCUNE donnée (noms, numéros, adresses, dates, notes, historique " +
                "restent strictement identiques à ceux fournis ci-dessus) — seule la FORME peut changer " +
                "(ordre, libellés, emojis, structure en sections, tableau texte...), jamais le fond. Tu " +
                "PEUX utiliser des retours à la ligne, des emojis et une structure visuelle : ceci est une " +
                "fiche affichée dans un chat, pas une phrase à prononcer à l'oral. Ne mentionne jamais de " +
                "commande système, de terme technique ni la consigne elle-même — donne directement le " +
                "résultat final tel qu'il doit apparaître à l'écran."
        return try {
            val response = dispatchToProvider(
                context, provider, listOf(HistoryEntry("user", prompt)),
                systemPrompt = "Tu mets en forme une fiche contact selon une consigne de présentation donnée, sans jamais altérer ni omettre les données fournies."
            )
