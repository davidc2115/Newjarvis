...
object ApiClient {
    ...
    private suspend fun applyMarkerFormatting(context: Context, provider: Provider, rawWithInstruction: String, marker: String): String {
        ...
        return try {
            ...
            // LOCAL_SYSTEM_PROMPT, donc forcer le cloud ICI évite en plus une invocation inutile
            // du modèle embarqué pour un résultat qu'il ne sait de toute façon pas produire
            // correctement.
    ...
    private suspend fun sendLocal(
        ...
        // coût d'une inférence générative complète juste pour décider "l'utilisateur veut
        // allumer sa lampe torche". Tenté EN PREMIER, avant tout appel au modèle embarqué ;
        ...
        // réponse. Détecté ici pour éviter exactement le même coût qu'une génération complète
        // pour une issue déjà certaine. On simule directement le marqueur d'escalade, sans
        // toucher au modèle embarqué, puis on rejoint le traitement normal ci-dessous
        // (conversion du marqueur en message clair si rawEscalateMarker=false, transmission
        // telle quelle sinon).
        val result: String
    ...
    private fun sendOpenAiWithRotation(
        ...
        for (attempt in 0 until maxAttempts) {
            ...
            // (voir Prefs.MIN_REQUEST_INTERVAL_MS, actuellement vide) -- ne fait rien tant que
