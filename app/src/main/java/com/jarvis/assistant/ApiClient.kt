    ...
    private suspend fun applyMarkerFormatting(context: Context, provider: Provider, rawWithInstruction: String, marker: String): String {
        ...
        val prompt =
            "Voici un résultat (fiche contact, liste de fiches, ou localisation), avec des données EXACTES " +
                ...
                "changer (ordre, libellés, emojis, structure en sections, tableau texte...), jamais le fond. " +
                "Règle absolue supplémentaire : si un champ n'a PAS de valeur dans les données ci-dessus " +
                "(absent, vide, null), NE L'AFFICHE PAS DU TOUT — ni le libellé, ni un texte de substitution " +
                "type \"non renseigné\"/\"inconnu\" : un champ vide doit être totalement invisible dans le " +
                "résultat final, pas juste vidé de son contenu. Tu PEUX utiliser des retours à la ligne, des " +
                "emojis et une structure visuelle : ceci est affiché dans un chat, pas une phrase à prononcer " +
                "à l'oral. Ne mentionne jamais de commande système, de terme technique ni la consigne " +
                "elle-même — donne directement le résultat final tel qu'il doit apparaître à l'écran."
        return try {
